resource "aws_security_group" "odin_lambda_sg" {
...
resource "aws_vpc_security_group_ingress_rule" "odin_lambda_sg_ingress_http" {
  ...
  security_group_id = aws_security_group.odin_lambda_sg.id
  from_port         = local.ports_http
  to_port           = local.ports_http
  cidr_ipv4         = local.all_ip_address
}

resource "aws_security_group" "odin_rds_sg" {
...
resource "aws_vpc_security_group_ingress_rule" "odin_rds_sg_ingress_mysql" {
  ip_protocol                  = local.tcp_protocol
  security_group_id            = aws_security_group.odin_rds_sg.id
  from_port                    = local.db_port
  to_port                      = local.db_port
  referenced_security_group_id = aws_security_group.odin_lambda_sg.id #allow traffic ONLY from specified SG
}

resource "aws_db_subnet_group" "odin_rds_subnet_group" {
